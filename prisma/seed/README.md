...
Patrick Dills is the Eugene lead processor, added per ADR-0023. He gets read access to Eugene site data and, **as of 2026-08-11, holds the Eugene bonus ops-signer slot** (Bill instruction — he replaced Shannon Rockwell, who had covered after Kelsey Ruhland's availability ended on 2026-08-08). This REVERSES the original ADR-0023 position that he be kept out of the chain for separation of duties: Patrick is himself a BonusEmployee at Eugene, so he can now be the default approver for amendments touching his own historical bonus rows. Bill accepted that trade-off explicitly; see ADR-0028. Like the other rows he ships `is_active=false` and is activated via the `/admin/users` panel after his first Entra SSO sign-in.

`bill.barnard@svdp.us` is seeded (added 2026-08-11) as Bill's real login identity and the universal bonus auto-override actor, `is_active=true`. It is distinct from `operations@svdp.us`, which is a deliberately-inactive import/automation alias. **Never point `bonus_signature_chains.csv` at `operations@svdp.us`** — the t3 08:30 PT auto-override refuses to sign as an inactive actor, which is what silently disabled the safety net at both sites and caused the 2026-07-07 missed Eugene payroll deadline.
