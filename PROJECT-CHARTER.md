...
- **Sprint 4 — prior-day amendment workflow + manager date picker + bi-site EOD check (2026-06-16, ADR-0028)** — **Four-eyes approval gate for prior-day bonus edits within the current `draft` period.** Morena Gomez asked (2026-06-15) what the correct process is to correct a prior day; there was none. Now a manager's prior-day `mattress_count` change or insert routes through a new `bonus_amendment_requests` ledger (new table + `BonusAmendmentRequestState`/`BonusAmendmentChangeType` enums, migration `20260616_amendment_workflow`, purely additive): it requires the counterpart signature-chain signer as approver (requester ≠ approver, DB CHECK + app mirror) and a ≥20-char justification (DB CHECK + app), with a **"Ping Bill" soft-control** escape that immediately adds the Director as an alternate eligible approver (abuse observable in the audit log). Approval atomically applies the entry change + writes the entry audit row + marks the request approved + links the audit id, all in one transaction. Bill is notified on **both** approve and reject (ntfy `dr3-vision-system` + M365 mail). No `delete` op (misattribution → set-to-0 + correct insert). Closed periods remain immutable to managers; the existing admin escape valve (`src/lib/bonus/amendment.ts`) is untouched; **no period state-machine changes**. The admin-only `AdminDatePicker` is replaced by `BonusDatePicker` (visible to all managers, constrained to the current period for managers and unconstrained for admins, defense-in-depth on client + `resolveEntryDate`). A bi-site EOD-check daemon (per-site missing-entry fingerprint) ships as a `bonus-eod-check` compose service. Patrick Dills is carved out (read-only) by the same separation-of-duties principle that excludes him from the Eugene signature chain. Built from `docs/sprints/sprint-4-handoff.md`; the spec's ADR-0027 was renumbered to **0028** (0027 was already taken by the PWA update-prompt shipped 2026-06-15). Full `vitest` suite green (825); `tsc --noEmit` 0; ESLint clean; `next build` succeeds. See ADR-0028 and the operator runbook `docs/operator/bonus-amendment-workflow.md`.
- **UI + PWA follow-ups (2026-06-15)** — Surfaced `employee_number` end-to-end in the Manage Employees UI (list + create + inline edit, app-level per-site uniqueness, en/es/ur, ADR-0026 follow-on). Added a PWA "update available — tap to reload" prompt (ADR-0027 — `skipWaiting:false` + waiting-SW detection + user-tapped promote, so a deploy never strands an open client on a stale shell). Fixed the daily-entry grid to repopulate immediately on date change (re-key `DailyEntryGrid` by `entryDate`; it previously kept the prior day's values until a manual reload because the `useState` seed only runs on mount).
- **Employee-number extraction (2026-06-15, ADR-0026)** — **Embedded employee numbers relocated out of `BonusEmployee.full_name` into a dedicated `employee_number` field.** A staff member had appended a trailing 4-digit number to the processor names (21 of 107 rows, all DR3 Woodland, uniform ` [0-9]{4}$` pattern) during the 2026-06-09/10 data work. Migration `20260615_bonus_employee_number` adds `employee_number String?` to `BonusEmployee` (with an `(site_id, employee_number)` index; app-level per-site uniqueness, no DB global unique per the Eugene/Woodland separation), extracts the trailing number into it, strips+trims `full_name`, and appends the pre-edit name to `previous_names` for provenance. Idempotent (re-run/retry-safe, guarded on `employee_number IS NULL`); non-matching rows untouched; never touches `audit_log`. Verified against the live prod DB inside a `BEGIN … ROLLBACK` transaction before merge — 21 extracted, 0 names left numbered, 0 duplicate groups, idempotent re-run touched 0 rows. The staff `users.name` table was already clean and is **not** modified; whether `users` should also carry an `employee_number` is deferred (QUESTIONS.md Q-2). No code parses numbers out of `full_name` (all consumers are display-only — behavior-neutral). Full `vitest` suite green (734); `tsc --noEmit` 0; ESLint clean. See ADR-0026, the plan `docs/plans/2026-06-15-bonus-employee-number-extraction.md`, and the CHANGELOG entry dated 2026-06-15.
- **Sprint 3 (2026-06-08 → 2026-06-09, ADR-0023)** — **Historical bonus data import + Period-13 production go-live.** 17 months of legacy processor-bonus data (Jan 2025 → Jun 2026) imported into Vision ahead of the Tue 2026-06-09 Period-13 go-live: **5,158 daily entries / 94 processors / 104 pay periods (76 `historical_imported`) reconciled to $113,776.00 to the cent**, plus 76/76 historical PDFs generated into R2. Delivered as a one-shot, SHA-256-idempotent seed (`seedHistoricalImport`) — **no bulk-upload UI** (design pivot, Q19/Q20; the dashboard bulk-upload tile was removed). New schema: enum value `historical_imported` (locked, amendment-only — not in `EDITABLE_STATES`), dual-total + `import_session_id` on `bonus_pay_periods`, `mattress_count` `Int → Decimal(5,1)`, per-row provenance, and the `bonus_imports` / `bonus_employee_aliases` tables. Daily-entry input now accepts one decimal place on both the entry and amendment write paths (shared `isValidMattressCount`). Governance: Patrick Dills seeded as a Eugene manager **and** `BonusEmployee` but deliberately **not** a Eugene signature-chain member (separation of duties). Fleet observability wired in (ADR-0022 — GlitchTip/Loki/Tempo/Grafana/Prometheus/ntfy, closing the T-018 deferral) and M365 Mail.Send made production-ready — both operator env config, not code flips. **Post-import operational fixes:** (a) **historical-PDF render hotfix** — `historical_imported` periods can predate the earliest `processor_bonus_rules` rule, so `resolveActiveRule` threw `NoActiveRuleError` and 0/76 PDFs rendered; new `resolveRuleForHistorical` falls back to the site's earliest rule for historical periods only (live periods stay strict; the displayed total is the stored as-paid legacy total per Q1). (b) **init reaper** — `init: true` on the `app`/`cron` compose services so a real PID-1 reaps the chromium/Playwright children the 76-PDF backfill leaked (~150 zombies). (c) **Bonus UX** — prominent **Manage Employees** + **Pay Period History** buttons on `/bonus` (history was unreachable), "month" → "Pay Period" nomenclature with real period labels ("Pay Period 13 · Jun 9–22, 2026"), and an explicit `max=today` date-picker hint. (d) **Staff activation** — kelsey/rick/janette/morena/patrick seeded `is_active=true` so the M365 group signs straight in through the Entra gate. Full `vitest` suite green (698); `tsc --noEmit` 0. See ADR-0023, ADR-0022, and the CHANGELOG entries dated 2026-06-08/09.
- **Sprint-2 addendum (2026-06-06)** — **Bonus cadence corrected to bi-weekly and Eugene enabled** as a second bonus site, both shipping on the Mon Jun 8, 2026 cutover. (1) **Bi-weekly cadence (ADR-0019.1).** The bonus system was originally specified monthly; verification against the SVdP 2026 Payroll Calendar showed pay periods are bi-weekly — 26/year, 14 days fixed, Tue→Mon, Friday pay date. The schema renamed `bonus_months → bonus_pay_periods` (with site-neutral `facility_*` / `ops_*` signature columns, `period_number` / `period_year` / `pay_date` / `*_auto_override_at` added, and a terminal `skipped` state), the close cron moved to a daily Pacific-aware 17:30 PT tick (old monthly `bonus-month-close` removed), and a hard **Tuesday 09:00 AM PT** payroll deadline was introduced — gated by an escalation cron (06:00 warn → 07:30 urgent → 08:30 **auto-override as Bill** → 09:00 deadline-miss check). Pre-cutover Period 12 of 2026 disposition: **skip** (admin-only `POST /api/bonus/months/[id]/skip`); Period 13 is the first canonical bi-weekly PDF. (2) **Eugene enablement (ADR-0019.2).** The site-scoped schema needed no migration; the work was the access matrix (Rick → Eugene only; Morena → Woodland only; admins → both with a site picker), a per-site `bonus_signature_chains` table (Eugene = Rick facility / Kelsey ops / Bill auto-override), PDF site-name substitution, and Vision Dashboard tile routing (Rick now sees the bonus tile). Both sites share the same pay-period calendar and the `payroll@svdp.us` destination. See ADR-0019.1, ADR-0019.2, and the cutover runbook `docs/operator/bonus-cadence-and-eugene-cutover.md`.
...
- **Facility manager — Woodland:** **Janette Tomas**. Per-site manager for Woodland operations.
...
7. **Site manager contact info.** Names captured: **Rick Albritton (Eugene)**, **Janette Tomas (Woodland)**. Email + cell needed at deployment for portal account setup; not blocking the build. Neither receives ntfy (Bill-only per ntfy policy).
