...
| CAPS    | No length caps on stored free-text (survey `answer_text`/`answer_json`, AP `note`/`vendor`) — storage-DoS boundary.                                                    | survey `draft/route.ts:7`, AP `decide/route.ts`  | `[FIXED (AP half)]` 2026-07-16 — AP `note`≤2000/`vendor`≤200, 400 on overflow (F7-AP). Survey caps → input/infra agent. |
