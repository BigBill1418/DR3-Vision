...
4. **`docs/adr/`** — read all 11 ADRs. They are short. They lock in technical decisions you should not re-litigate.
...
2. **Eugene and Woodland are strictly separated.** Different MRC contracts, different jurisdictions (Oregon vs California), different rates, different storage limits, different retention rules. Every query, list, export, alert is scoped to one site by default. Cross-site rollups require `admin` role.
