...
**Resolution:** ANSWERED — **option (a)**, resolved by the **ADR-0037 "§3 amendment — D6 physical-snapshot pool split" (planning rollup 2026-07-08 §1.4)**, which states "This resolves the Q-3." `site_inventory_snapshots` now carries `program_units` / `non_program_units` + `pool_attribution ('measured' | 'legacy')` (migration `20260715_pool_split`): a `'measured'` anchor supplies the anchor pool pair to `onHand()` (validated `program + non_program === total`, typed `PoolSplitMismatchError`/422); a `'legacy'` anchor (pre-amendment rows, backfilled `program_units = total`, `non_program_units = 0`) keeps the attributed-all-to-program behavior this question proposed. Either way `program + nonProgram === total` holds and `computeRunningBalance`'s return shape is unchanged. Later refinement from historical workbook pool data (ADR-0039) remains possible without a schema change.
