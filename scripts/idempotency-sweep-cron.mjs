#!/usr/bin/env node
// ADR-0078 — idempotency-key TTL sweep (thin Pacific scheduler).
//
// Structurally identical to scripts/audit-sweep-cron.mjs, deliberately: this
// .mjs imports NOTHING (no @prisma/client, no tsx, no TS), computes the next
// fire instant DST-correctly via Intl formatToParts, sleeps, POSTs the internal
// loopback-guarded route with the `INTERNAL_CRON_TOKEN` bearer, and loops.
//
// Fires at 03:10 America/Los_Angeles — inside the quiet window, and offset from
// the 02:30 audit sweep so the two are not contending for the same connections.
//
// The route does the work and owns the retention floor (7 days). That floor is
// deliberately NOT a parameter here: a sweep whose horizon the caller chooses is
// one bad argument away from deleting live keys, and a deleted key means the
// next replay of an in-flight write is treated as a first submission.
//
// Deleting nothing is the normal outcome and is not an error. These rows are
// receipts, and a quiet night is a night on which no receipt aged out.
...
function logTs(message) {
  console.log(`[idempotency-sweep-cron ${new Date().toISOString()}] ${message}`);
}
...
async function runFireOnce() {
  ...
  const res = await fetch(`${BASE}/api/internal/idempotency/sweep`, {
    method: 'POST',
    headers,
    redirect: 'manual',
  });
...
async function tick() {
  ...
  logTs(`next idempotency sweep at ${fire.toISOString()} (in ${(sleepMs / 1000 / 60).toFixed(1)}min)`);
  await sleep(sleepMs);
  if (stopping) return;
...
async function main() {
  setupShutdown();
  logTs('daemon starting — idempotency TTL sweep anchored to 03:10 America/Los_Angeles');
...
if (isEntrypoint) {
  main().catch((err) => {
    console.error('idempotency-sweep-cron: fatal', err);
    process.exit(1);
  });
}
