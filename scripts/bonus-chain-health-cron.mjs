#!/usr/bin/env node
// ADR-0019.4 — bonus signature-chain health check (thin Pacific scheduler).
//
// Structurally identical to scripts/idempotency-sweep-cron.mjs, deliberately:
// this .mjs imports NOTHING (no @prisma/client, no tsx, no TS), computes the
// next fire instant DST-correctly via Intl formatToParts, sleeps, POSTs the
// internal loopback-guarded route with the `INTERNAL_CRON_TOKEN` bearer, and
// loops.
//
// Fires at 06:30 America/Los_Angeles. The time is not arbitrary: the escalation
// ladder's first tier runs at 07:10 and its auto-override at 08:30, so this lands
// FORTY MINUTES BEFORE the machinery that depends on the chain starts running.
// A chain found broken here is still fixable by hand before the 09:00 payroll
// deadline; a chain found broken by the auto-override itself is not, which is
// what happened on 2026-07-07 and again on 2026-08-04.
//
// Running daily rather than only on payroll mornings is the point. The 2026-08-04
// break was introduced by a `db:seed` run days earlier and sat undetected because
// nothing looked at the chain until the moment it was needed.
//
// A green result is the normal outcome and is not interesting. The route owns
// the ledger write and the paging decision; this wrapper owns only "when".
...
// Fixed daily fire: 03:10 America/Los_Angeles (ADR-0078).
const FIRE_HOUR = 6;
...
function logTs(message) {
  console.log(`[bonus-chain-health-cron ${new Date().toISOString()}] ${message}`);
}
...
async function runFireOnce() {
  ...
  const res = await fetch(`${BASE}/api/internal/bonus/chain-health`, {
    method: 'POST',
    headers,
    redirect: 'manual',
  });
...
async function tick() {
  ...
  logTs(`next chain health check at ${fire.toISOString()} (in ${(sleepMs / 1000 / 60).toFixed(1)}min)`);
  await sleep(sleepMs);
  if (stopping) return;

  const text = await runFireOnce();
  logTs(`chain health check complete: ${text}`);
}

async function main() {
  setupShutdown();
  logTs('daemon starting — signature-chain health check anchored to 06:30 America/Los_Angeles');

  while (!stopping) {
    try {
      await tick();
    } catch (err) {
      logTs(`check failed (non-fatal, retry next iteration): ${err?.message ?? err}`);
      await sleep(30_000);
    }
  }
}
...
if (isEntrypoint) {
  main().catch((err) => {
    console.error('bonus-chain-health-cron: fatal', err);
    process.exit(1);
  });
}
