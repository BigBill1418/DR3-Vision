// Type surface for the (JS) ADR-0058 processed → inventory bridge backfill
// runner's pure helpers + injectable orchestrator, so a vitest test can import
// them under `allowJs: false`. The runner auto-start is guarded behind an
// entrypoint check, so importing the module is side-effect-free (the only
// consumer of these types is the test).
...
export interface BackfillDeps {
  /** The compiled `@/lib/mymrc` surface (bridgeProcessedToInventory, ntfyPager). */
  mymrc: unknown;
  /** A PrismaClient. */
  prisma: unknown;
...
/**
 * Run the one-shot backfill with the MANDATORY floor-invariance gate and RESOLVE
 * the process exit code (never calls `process.exit`): 0 success (or --dry-run) ·
 * 1 the live floor drifted (paged) or a probe/site-resolve failed.
 */
export function runProcessedBridgeBackfill(deps: BackfillDeps): Promise<number>;
