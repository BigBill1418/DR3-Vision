#!/usr/bin/env node
// ADR-0058 — ONE-SHOT processed → inventory bridge backfill.
//
// The hourly scrape (`scripts/mymrc-scrape.mjs`) only bridges a trailing ~10-day
// window. THIS runner bridges the FULL history (2023 → today) once, so
// `processed_units_daily` (empty before ADR-0058) gains every Woodland processed
// day. It needs NO MyMRC login/browser — it only reads the already-synced
// `mymrc_processed_mirror` and writes `processed_units_daily` via the compiled
// bundle (`dist/mymrc`).
//
// MONEY-SAFE GO-LIVE GATE (ADR-0058 D4, MANDATORY): every backfilled day is ≤ the
// latest physical anchor, so `onHand` (`{ gt: anchorDay }`) excludes them and the
// LIVE FLOOR must not move. This runner PROVES that in practice: it probes
// `onHand(site, asOf)` via the internal floor-probe route BEFORE and AFTER the
// write (same `asOf`), and ABORTS non-zero if the floor drifted by a single unit.
//
// Run it from the APP container (it has INTERNAL_CRON_TOKEN + can reach the
// internal route on 127.0.0.1:3000, and the shared image carries dist/mymrc):
//   docker compose exec app node scripts/mymrc-processed-bridge-backfill.mjs --backfill
// Flags:
...
//   1 — floor drifted (INVESTIGATE — a processed_date encoding bug), a probe failed,
//       or an unhandled fatal error.
...
function log(level, message) {
  const line = `mymrc-processed-bridge-backfill[${new Date().toISOString()}]: ${message}`;
...
/**
 * Run the one-shot backfill with the mandatory floor-invariance gate. Collaborators
 * are INJECTED so the flow is unit-testable with fakes:
 *   - mymrc:      the compiled `@/lib/mymrc` surface (bridgeProcessedToInventory).
 *   - prisma:     a PrismaClient (mirror read + pud write + site resolve).
 *   - probe:      `(siteCode, asOfIso) => Promise<{program,nonProgram,total}>`.
 *   - opts:       parsed args.
 * Resolves the process exit code; the caller owns `process.exit`.
 */
export async function runProcessedBridgeBackfill({ mymrc, prisma, probe, opts, log: logFn = log }) {
  ...
  const sinceProductionDate = opts.since ? new Date(`${opts.since}T00:00:00.000Z`) : undefined;
  ...
  if (opts.dryRun) {
    ...
    logFn(
      ...
      `DRY-RUN — would: days=${res.daysConsidered} ins=${res.inserted} upd=${res.updated} ` +
        `skip=${res.skippedGuarded} same=${res.unchanged} (NO writes, NO floor gate)`,
    );
    return 0;
  }

  // ── MANDATORY floor-invariance gate (ADR-0058 D4) ──
  // One fixed asOf for BOTH probes so the comparison isolates the bridge's writes
  // from clock advance.
  ...
  logFn(
    ...
    `bridge wrote: days=${res.daysConsidered} ins=${res.inserted} upd=${res.updated} ` +
      `skip=${res.skippedGuarded} same=${res.unchanged}`,
  );
    ...
    for (const c of drifted) {
      logFn(
        ...
        `LIVE FLOOR DRIFTED for ${c}: before ${before[c].program}/${before[c].nonProgram}/${before[c].total} ` +
          `!= after ${after[c].program}/${after[c].nonProgram}/${after[c].total}. A bridged row landed at/after ` +
          `the anchor unexpectedly (processed_date encoding bug) — INVESTIGATE before trusting inventory. ` +
          `(Expected non-zero drift ONLY if post-anchor production days were bridged.)`,
      );
    }
    ...
    if (typeof mymrc.ntfyPager?.page === 'function') {
      await mymrc.ntfyPager
        .page({
          kind: 'error',
          site: 'admin',
          message: `processed-bridge backfill: LIVE FLOOR DRIFTED for ${drifted.join(', ')} — anchor-safety gate FAILED. Investigate.`,
          fingerprint: 'processed-bridge-floor-drift',
        })
        .catch(() => undefined);
    }
    return 1;
