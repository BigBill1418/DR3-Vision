...
4. **`docs/adr/`** — read all ADRs (**95 files as of 2026-08-10**, re-counted: 0001–0090 plus 0019.1/0019.2 and a few split/duplicate numbers; see `docs/adr/README.md` for the index). The count is `ls docs/adr/*.md | wc -l` minus the README. They are short. They lock in technical decisions you should not re-litigate. **Amendments are sections INSIDE their ADR, not separate files** — 0077 carries Amendments 1, 2 and 3; 0070, 0074, 0078, 0079, 0083, 0084, 0086, 0089 and 0090 each carry Amendment 1; none of them adds to the file count.
   - **0086** (capture-time photo upload grants) was accepted by Bill at the 2026-08-08 walkthrough and is implemented; it ships INERT until the operator provisions `PHOTO_GRANT_SECRET`. Its §4 records why the earlier §0.AJ design was unbuildable and its §10 records two places the ADR text disagreed with the live code. **Am.1 (2026-08-10)** — an expired session answered 403 and the reject stage said "retry" forever; a refused mint now answers 401 and the screen says so.
   - **0083** (saves are paid units), **0084** (same-day physical-count void) and **0085** (the walk-up drop-off is a label, a count and a photo) landed 2026-08-08 — the P3/P4/P5 phases of the #205 floor handoff.
   - **0087** (VLM equipment identity + normalization + the decision register) is **Proposed and NOT implemented** — decision-ready for Bill. Do not treat it as in force.
   - **0088** (throughput-gap watchdog) shipped born-pilot 2026-08-08 and was **ramped LIVE at Woodland 2026-08-10**. Eugene stays `pilot` by design — no machine there, so its scan can never fire.
   - **0089** (inbound is keyed on a scheduling field) landed 2026-08-10 and is the one to read before touching MyMRC ingestion: inbound was keyed on `Docking_Appointment_Date__c`, a SCHEDULING field that is null on every route-collection haul. The true key is `Recycler_Reported_Delivery_Date__c`; the bridge and `freshness.ts` now key on `COALESCE(that, appointment date)`. **It falsifies ADR-0059's "undated hauls are all pre-anchor and inert / the live path is fully covered" tradeoff.** Am.1 proves the field live. The D4 recovery RAN the same day: 7,314/7,314 dated, floor −1,671 → **+1,382**, July Woodland COR unblocked.
   - **0090** (floor workflow ergonomics) landed 2026-08-10 in two PRs — A + C (#226: haul numbers on five operator surfaces; `voided` as a new `LoadStatus` MEMBER, deliberately inverting ADR-0084's column pair, with the `expected_loads` slot SEVERED) and **B under Amendment 1** (#227: back navigation, weight correction that APPENDS an audit row, stack soft-void, `finished → in_progress` reopen with `unload_duration_seconds` FROZEN at the first finish — Bill's call).
   - ADR-0012 in particular bundles seven Sprint-1 clarifications that supplement ADRs 0001/0004/0008/0009; the bonus system spans ADR-0019/0019.1/0019.2/0020/0021/0022/0023/0025 (0025 hardens the ADR-0019.1 escalation tier-4 alert).
...
7. **Photos go to Cloudflare R2 via signed URLs; never store them in the application database or on the fleet host's local disk.** (R2 is no longer the _only_ copy of them: since 2026-09-11/12 the fleet's `fleetbackup-r2-mirror` lane copy-forwards every R2 bucket — `dr3-vision-photos` included — into Backblaze B2 with Object Lock compliance 90 d and no pruning, and `fleetbackup-chad` takes svdp-dev's whole root including the live Postgres volume. noc-master ADR-0232; see `docs/operator/backups.md`. This changes nothing about the rule — do not start writing photos to the DB or host disk.)
